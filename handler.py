...
"""
=============================================================================
GOOGLE APPS SCRIPT V13 - THUMBNAIL VERSION
=============================================================================

Copy this to Google Apps Script:

/**
 * Google Apps Script V13 - Thumbnail Upload with Padding Fix
 */

function doPost(e) {
  try {
    console.log('V13 Thumbnail Upload started');
    
    const postData = JSON.parse(e.postData.contents);
    console.log('Input keys:', Object.keys(postData));
    
    // Find thumbnail data
    let base64Data = postData.thumbnail || postData.image || postData.data;
    ...
    const imageBlob = Utilities.newBlob(
      ...
      'image/png',
      `thumbnail_ring_${new Date().getTime()}.png`
    );
    ...
    return ContentService
      .createTextOutput(JSON.stringify({
        ...
        message: 'V13 Thumbnail uploaded successfully'
      }))
...
1. Set REPLICATE_API_TOKEN environment variable in RunPod
2. Deploy both handlers to RunPod
3. Update Google Apps Scripts with V13 code
4. Configure Make.com paths:
   - Enhancement: {{4.data.output.output.enhanced_image}}
   - Thumbnail: {{4.data.output.output.thumbnail}}

Key improvements in V13:
- Enhanced black frame detection (multiple thresholds)
- Improved FLUX Fill integration
- Better ring detection and centering
- Consistent enhancement between handlers
